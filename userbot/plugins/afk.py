    ...
    if AFK_.USERAFK_ON and not (await event.get_sender()).bot:
        ...
        if AFK_.afk_type == "text":
            if AFK_.msg_link and AFK_.reason:
                message_to_reply = (
                    f"**Sorry man!My baby is busy now.\nShe is in my bed.\nPlease don't disturb.\nShe'll catch up with u later .\n\nAway Since {endtime}\nReason : **{AFK_.reason}"
                )
            elif AFK_.reason:
                message_to_reply = (
                    f"`Sorry man!My baby is busy now.\nShe is in my bed.\nPLease don't disturb.\nShe'll catch up with u later .\n\nAway Since {endtime}\nReason : {AFK_.reason}`"
                ...
                message_to_reply = f"`Sorry man!My baby is busy now.\nShe is in my bed.\nPLease don't disturb.\nShe'll catch up with u later .\n\nAway Since {endtime}\nReason : Not Mentioned ( ಠ ʖ̯ ಠ)`"
...
async def _(event):
    ...
    if not AFK_.USERAFK_ON:
        ...
        if AFK_.reason:
            await edit_delete(
                event, f"`Sorry man!My baby is busy now.She'll catch up with u later! because ~` {AFK_.reason}", 5
            )
        else:
            await edit_delete(event, f"`Sorry man!My baby is busy now.She'll catch up with u later! `", 5)
        if BOTLOG:
            if AFK_.reason:
                await event.client.send_message(
                    BOTLOG_CHATID,
